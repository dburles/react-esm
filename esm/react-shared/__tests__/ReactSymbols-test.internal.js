...
describe('ReactSymbols', () => {
  ...
  const expectToBeUnique = keyValuePairs => {
    ...
    keyValuePairs.forEach(([key, value]) => {
      if (map.has(value)) {
        throw Error(`${key} value ${value.toString()} is the same as ${map.get(value)}.`);
      }
  ...
  it('Symbol values should be unique', () => {
    expectToBeUnique(Object.entries(require("../ReactSymbols")));
  });
  it('numeric values should be unique', () => {
    ...
    try {
      const entries = Object.entries(require("../ReactSymbols")).filter( // REACT_ASYNC_MODE_TYPE and REACT_CONCURRENT_MODE_TYPE have the same numeric value
