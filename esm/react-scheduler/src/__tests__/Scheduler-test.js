...
describe('Scheduler', () => {
  beforeEach(() => {
    ...
    window._schedMock = [requestHostCallback, cancelHostCallback, shouldYieldToHost, getCurrentTime];
  ...
  it('flushes work until framesize reached', () => {
    ...
    expect(Date.now()).toEqual(0); // No time left after A1_100 and A2_200 are run

    expect(flushWork(300)).toEqual(['A1_100', 'A2_200']);
    expect(Date.now()).toEqual(300); // B2_200 is started as there is still time left after B1_100

    expect(flushWork(101)).toEqual(['B1_100', 'B2_200']);
    expect(Date.now()).toEqual(600); // C1_300 is started as there is even a little frame time

    expect(flushWork(1)).toEqual(['C1_300']);
    expect(Date.now()).toEqual(900); // C2_300 is started even though there is no frame time

    expect(flushWork(0)).toEqual(['C2_300']);
    expect(Date.now()).toEqual(1200); // D_3000 is very slow, but won't affect next flushes (if no
    // timeouts happen)

    expect(flushWork(100)).toEqual(['D_3000']);
    expect(Date.now()).toEqual(4200);
    expect(flushWork(400)).toEqual(['E1_300', 'E2_200']);
    expect(Date.now()).toEqual(4700); // Default timeout is 5000, so during F2_200, work will timeout and are done
    // in reverse, including F2_200

    expect(flushWork(1000)).toEqual(['F1_200', 'F2_200', 'F3_300', 'F4_500', 'F5_200', 'F6_20']);
  ...
  it('cancels work', () => {
    ...
    expect(flushWork()).toEqual(['A', // B should have been cancelled
    'C']);
  });
  it('executes the highest priority callbacks first', () => {
    scheduleCallback(() => doWork('A', 100));
    scheduleCallback(() => doWork('B', 100)); // Yield before B is flushed
  ...
  it('expires work', () => {
    ...
    advanceTime(249);
    expect(clearYieldedValues()).toEqual([]); // Advance by just a bit more to expire the high pri callbacks

    advanceTime(1);
    expect(clearYieldedValues()).toEqual(['B', 'D']); // Expire the rest
  ...
  it('continuation callbacks inherit the expiration of the previous callback', () => {
    ...
    runWithPriority(UserBlockingPriority, () => scheduleCallback(work)); // Flush until just before the expiration time

    expect(flushWork(249)).toEqual(['A', 'B', 'Yield!']); // Advance time by just a bit more. This should expire all the remaining work.
  ...
  it('nested callbacks inherit the priority of the currently executing callback', () => {
    ...
    expect(flushWork(100)).toEqual(['Parent callback']); // The nested callback has user-blocking priority, so it should
    // expire quickly.
  ...
  it('continutations are interrupted by higher priority work scheduled ' + 'inside an executing callback', () => {
    const tasks = [['A', 100], ['B', 100], ['C', 100], ['D', 100]];
    ...
    const work = () => {
      while (tasks.length > 0) {
        ...
        if (task[0] === 'B') {
          ...
          runWithPriority(UserBlockingPriority, () => scheduleCallback(() => doWork('High pri', 100)));
        }
    ...
    scheduleCallback(work);
    expect(flushWork()).toEqual(['A', 'B', 'Schedule high pri', // Even though there's time left in the frame, the low pri callback
    // should yield to the high pri callback
    'Yield!', 'High pri', // Continue low pri work
    'C', 'D']);
  });
  it('immediate callbacks fire at the end of outermost event', () => {
    runWithPriority(ImmediatePriority, () => {
      scheduleCallback(() => yieldValue('A'));
      scheduleCallback(() => yieldValue('B')); // Nested event

      runWithPriority(ImmediatePriority, () => {
        scheduleCallback(() => yieldValue('C')); // Nothing should have fired yet
  ...
  it('wrapped callbacks inherit the current priority', () => {
    ...
    const wrappedInteractiveCallback = runWithPriority(UserBlockingPriority, () => wrapCallback(() => {
      scheduleCallback(() => {
        doWork('User-blocking', 100);
      });
    })); // This should schedule a normal callback

    wrappedCallback(); // This should schedule an user-blocking callback
  ...
  it('immediate callbacks fire at the end of callback', () => {
    const immediateCallback = runWithPriority(ImmediatePriority, () => wrapCallback(() => {
      scheduleCallback(() => yieldValue('callback'));
    }));
    immediateCallback(); // The callback was called at the end of the outer event
  ...
  it("immediate callbacks fire even if there's an error", () => {
    ...
    expect(clearYieldedValues()).toEqual(['A']); // B and C flush in a subsequent event. That way, the second error is not
    // swallowed.
  ...
  it('exposes the current priority level', () => {
    ...
    expect(clearYieldedValues()).toEqual([NormalPriority, ImmediatePriority, NormalPriority, UserBlockingPriority, ImmediatePriority]);
  });
});