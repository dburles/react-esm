...
describe('Scheduling UMD bundle', () => {
  ...
  function validateForwardedAPIs(api, forwardedAPIs) {
    ...
    forwardedAPIs.forEach(forwardedAPI => {
      expect(Object.keys(forwardedAPI).filter(filterPrivateKeys).sort()).toEqual(apiKeys);
    });
  }
  ...
  it('should define the same tracing API', () => {
    ...
    validateForwardedAPIs(api, [umdAPIDev, umdAPIProd, umdAPIProfiling, secretAPI.SchedulerTracing]);
  });
});