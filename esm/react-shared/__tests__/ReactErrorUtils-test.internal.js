...
describe('ReactErrorUtils', () => {
  ...
  it(`handles nested errors`, () => {
    ...
    ReactErrorUtils.invokeGuardedCallback('foo', function () {
      ...
      err2 = ReactErrorUtils.clearCaughtError();
    }, null); // Returns null because inner error was already captured
  ...
  it('handles nested errors in separate renderers', () => {
    ...
    let ops = [];
    ReactErrorUtils1.invokeGuardedCallback(null, () => {
      ReactErrorUtils2.invokeGuardedCallback(null, () => {
        throw new Error('nested error');
      }, null); // ReactErrorUtils2 should catch the error

      ops.push(ReactErrorUtils2.hasCaughtError());
      ops.push(ReactErrorUtils2.clearCaughtError().message);
    }, null); // ReactErrorUtils1 should not catch the error
  ...
  if (!__DEV__) {
    ...
    it('catches null values', () => {
      ReactErrorUtils.invokeGuardedCallback(null, function () {
  ...
  it(`can be shimmed`, () => {
    const ops = [];
    jest.resetModules();
    jest.mock("../invokeGuardedCallbackImpl", () => function invokeGuardedCallback(name, func, context, a) {
      ops.push(a);
    ...
    try {
      ...
      jest.unmock("../invokeGuardedCallbackImpl");
    }
  });
});