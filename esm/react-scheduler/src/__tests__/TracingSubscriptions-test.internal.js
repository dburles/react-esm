...
describe('TracingSubscriptions', () => {
  ...
  function loadModules({
    ...
    jest.useFakeTimers();
    currentTime = 0;
    ReactFeatureFlags = require("../../../react-shared/ReactFeatureFlags");
    ReactFeatureFlags.enableSchedulerTracing = enableSchedulerTracing;
    SchedulerTracing = require("../../tracing");
  ...
  describe('enabled', () => {
    ...
    describe('error handling', () => {
      it('should cover onInteractionTraced/onWorkStarted within', done => {
        SchedulerTracing.unstable_trace(firstEvent.name, currentTime, () => {
          const mock = jest.fn(); // It should call the callback before re-throwing

          throwInOnInteractionTraced = true;
          expect(() => SchedulerTracing.unstable_trace(secondEvent.name, currentTime, mock, threadID)).toThrow('Expected error onInteractionTraced');
          throwInOnInteractionTraced = false;
          expect(mock).toHaveBeenCalledTimes(1);
          throwInOnWorkStarted = true;
          expect(() => SchedulerTracing.unstable_trace(secondEvent.name, currentTime, mock, threadID)).toThrow('Expected error onWorkStarted');
          expect(mock).toHaveBeenCalledTimes(2); // It should restore the previous/outer interactions

          expect(SchedulerTracing.unstable_getCurrent()).toMatchInteractions([firstEvent]); // It should call other subscribers despite the earlier error
      ...
      it('should cover onWorkStopped within trace', done => {
        SchedulerTracing.unstable_trace(firstEvent.name, currentTime, () => {
          ...
          const mock = jest.fn(() => {
            innerInteraction = Array.from(SchedulerTracing.unstable_getCurrent())[1];
          });
          throwInOnWorkStopped = true;
          expect(() => SchedulerTracing.unstable_trace(secondEvent.name, currentTime, mock)).toThrow('Expected error onWorkStopped');
          throwInOnWorkStopped = false; // It should restore the previous/outer interactions

          expect(SchedulerTracing.unstable_getCurrent()).toMatchInteractions([firstEvent]); // It should update the interaction count so as not to interfere with subsequent calls

          expect(innerInteraction.__count).toBe(0); // It should call other subscribers despite the earlier error
      ...
      it('should cover onInteractionScheduledWorkCompleted within trace', done => {
        SchedulerTracing.unstable_trace(firstEvent.name, currentTime, () => {
          ...
          expect(() => SchedulerTracing.unstable_trace(secondEvent.name, currentTime, mock)).toThrow('Expected error onInteractionScheduledWorkCompleted');
          throwInOnInteractionScheduledWorkCompleted = false; // It should restore the previous/outer interactions
      ...
      it('should cover onWorkScheduled within wrap', done => {
        SchedulerTracing.unstable_trace(firstEvent.name, currentTime, () => {
          const interaction = Array.from(SchedulerTracing.unstable_getCurrent())[0];
          const beforeCount = interaction.__count;
          throwInOnWorkScheduled = true;
          expect(() => SchedulerTracing.unstable_wrap(() => {})).toThrow('Expected error onWorkScheduled'); // It should not update the interaction count so as not to interfere with subsequent calls

          expect(interaction.__count).toBe(beforeCount); // It should call other subscribers despite the earlier error
      ...
      it('should cover onWorkStarted within wrap', () => {
        ...
        throwInOnWorkStarted = true;
        expect(wrapped).toThrow('Expected error onWorkStarted'); // It should call the callback before re-throwing

        expect(mock).toHaveBeenCalledTimes(1); // It should update the interaction count so as not to interfere with subsequent calls

        expect(interaction.__count).toBe(0); // It should call other subscribers despite the earlier error

        expect(secondSubscriber.onWorkStarted).toHaveBeenCalledTimes(2);
      });
      it('should cover onWorkStopped within wrap', done => {
        SchedulerTracing.unstable_trace(firstEvent.name, currentTime, () => {
          const outerInteraction = Array.from(SchedulerTracing.unstable_getCurrent())[0];
          ...
          throwInOnWorkStopped = false; // It should restore the previous interactions

          expect(SchedulerTracing.unstable_getCurrent()).toMatchInteractions([outerInteraction]); // It should update the interaction count so as not to interfere with subsequent calls
      ...
      it('should cover onWorkCanceled within wrap', () => {
        ...
        expect(onWorkCanceled).toHaveBeenCalledTimes(1); // It should update the interaction count so as not to interfere with subsequent calls

        expect(interaction.__count).toBe(0);
        expect(onInteractionScheduledWorkCompleted).toHaveBeenLastNotifiedOfInteraction(firstEvent); // It should call other subscribers despite the earlier error
    ...
    it('calls lifecycle methods for trace', () => {
      ...
      SchedulerTracing.unstable_trace(firstEvent.name, currentTime, () => {
        ...
        SchedulerTracing.unstable_trace(secondEvent.name, currentTime, () => {
          ...
          expect(onInteractionScheduledWorkCompleted).not.toHaveBeenCalled();
          expect(onWorkStarted).toHaveBeenCalledTimes(2);
          expect(onWorkStarted).toHaveBeenLastNotifiedOfWork(new Set([firstEvent, secondEvent]), threadID);
          expect(onWorkStopped).not.toHaveBeenCalled();
        }, threadID);
        expect(onInteractionScheduledWorkCompleted).toHaveBeenCalledTimes(1);
        expect(onInteractionScheduledWorkCompleted).toHaveBeenLastNotifiedOfInteraction(secondEvent);
        expect(onWorkStopped).toHaveBeenCalledTimes(1);
        expect(onWorkStopped).toHaveBeenLastNotifiedOfWork(new Set([firstEvent, secondEvent]), threadID);
      }, threadID);
      ...
      expect(onWorkStarted).toHaveBeenCalledTimes(2);
      expect(onWorkStopped).toHaveBeenCalledTimes(2);
      expect(onWorkStopped).toHaveBeenLastNotifiedOfWork(new Set([firstEvent]), threadID);
    });
    it('calls lifecycle methods for wrap', () => {
      ...
      SchedulerTracing.unstable_trace(firstEvent.name, currentTime, () => {
        ...
        SchedulerTracing.unstable_trace(secondEvent.name, currentTime, () => {
          ...
          expect(onWorkScheduled).toHaveBeenLastNotifiedOfWork(new Set([firstEvent, secondEvent]), threadID);
        });
      });
      expect(onInteractionTraced).toHaveBeenCalledTimes(2);
      expect(onInteractionScheduledWorkCompleted).not.toHaveBeenCalled();
      ...
      expect(onWorkStopped).toHaveBeenLastNotifiedOfWork(new Set([firstEvent, secondEvent]), threadID);
      expect(onInteractionScheduledWorkCompleted.mock.calls[0][0]).toMatchInteraction(firstEvent);
      expect(onInteractionScheduledWorkCompleted.mock.calls[1][0]).toMatchInteraction(secondEvent);
    });
    it('should call the correct interaction subscriber methods when a wrapped callback is canceled', () => {
      ...
      expect(onWorkCanceled).toHaveBeenLastNotifiedOfWork(new Set([firstEvent, secondEvent]), threadID);
      wrappedOne.cancel();
      expect(onInteractionScheduledWorkCompleted).toHaveBeenCalledTimes(2);
      expect(onInteractionScheduledWorkCompleted).toHaveBeenLastNotifiedOfInteraction(firstEvent);
      expect(onWorkCanceled).toHaveBeenCalledTimes(2);
      expect(onWorkCanceled).toHaveBeenLastNotifiedOfWork(new Set([firstEvent]), threadID);
