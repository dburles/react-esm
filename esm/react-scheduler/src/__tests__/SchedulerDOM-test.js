...
describe('SchedulerDOM', () => {
  ...
  function runPostMessageCallbacks(config) {
  ...
  function advanceOneFrame(config = {}) {
    runRAFCallbacks();
  ...
  describe('scheduleCallback', () => {
    ...
    it('inserts its rAF callback as early into the queue as possible', () => {
      ...
      expect(rAFCallbacks.length).toBe(2); // The user callback should be the second callback.
    ...
    describe('with multiple callbacks', () => {
      it('accepts multiple callbacks and calls within frame when not blocked', () => {
        ...
        scheduleCallback(callbackA); // initially waits to call the callback

        expect(callbackLog).toEqual([]); // waits while second callback is passed

        scheduleCallback(callbackB);
        expect(callbackLog).toEqual([]); // after a delay, calls as many callbacks as it has time for
      ...
      it("accepts callbacks between animationFrame and postMessage and doesn't stall", () => {
        ...
        runRAFCallbacks(); // this should schedule work *after* the requestAnimationFrame but before the message handler

        scheduleCallback(callbackB);
        expect(callbackLog).toEqual([]); // now it should drain the message queue and do all scheduled work

        runPostMessageCallbacks({
          timeLeftInFrame: 15
        });
        expect(callbackLog).toEqual(['A', 'B']); // advances timers, now with an empty queue of work (to ensure they don't deadlock)
      ...
      it('schedules callbacks in correct order and' + 'keeps calling them if there is time', () => {
        const {
          unstable_scheduleCallback: scheduleCallback
        } = Scheduler;
        const callbackLog = [];
        ...
        expect(callbackLog).toEqual([]); // continues waiting while B is scheduled

        scheduleCallback(callbackB);
        expect(callbackLog).toEqual([]); // after a delay, calls the scheduled callbacks,
      ...
      it('schedules callbacks in correct order when callbacks have many nested scheduleCallback calls', () => {
        ...
        scheduleCallback(callbackB); // initially waits to call the callback

        expect(callbackLog).toEqual([]); // while flushing callbacks, calls as many as it has time for
      ...
      it('schedules callbacks in correct order when they use scheduleCallback to schedule themselves', () => {
        ...
        expect(callbackLog).toEqual([]);
        scheduleCallback(callbackB);
        expect(callbackLog).toEqual([]); // after a delay, calls the latest callback passed
    ...
    describe('when callbacks time out: ', () => {
      ...
      describe('when there is no more time left in the frame', () => {
        it('calls any callback which has timed out, waits for others', () => {
          ...
          const callbackLog = []; // simple case of one callback which times out, another that won't.
          ...
          advanceOneFrame({
            timePastFrameDeadline: 16
          }); // callbackC should have timed out

          expect(callbackLog).toEqual(['C']); // push time ahead a bit so that we have no idle time

          advanceOneFrame({
            timePastFrameDeadline: 16
          }); // callbackB should have timed out

          expect(callbackLog).toEqual(['C', 'B']); // let's give ourselves some idle time now
      ...
      describe('when there is some time left in the frame', () => {
        it('calls timed out callbacks and then any more pending callbacks, defers others if time runs out', () => {
          ...
          startOfLatestFrame = 1000000000000;
          currentTime = startOfLatestFrame - 10;
          const callbackLog = []; // simple case of one callback which times out, others that won't.

          const callbackA = jest.fn(() => {
            callbackLog.push('A'); // time passes, causing us to run out of idle time
          ...
          advanceOneFrame({
            timeLeftInFrame: 15
          }); // runs rAF and postMessage callbacks
          // callbackC should have timed out
          // we should have had time to call A also, then we run out of time

          expect(callbackLog).toEqual(['C', 'A']); // push time ahead a bit so that we have no idle time
  ...
  describe('cancelCallback', () => {
    ...
    describe('with multiple callbacks', () => {
      it('when called more than once', () => {
        ...
        cancelCallback(callbackId);
        cancelCallback(callbackId);
        cancelCallback(callbackId); // Initially doesn't call anything
      ...
      it('when one callback cancels the next one', () => {
        ...
        const callbackB = jest.fn(() => callbackLog.push('B'));
        scheduleCallback(callbackA);
        callbackBId = scheduleCallback(callbackB); // Initially doesn't call anything
  ...
  describe('when callbacks throw errors', () => {
    describe('when some callbacks throw', () => {
      ...
      it('still calls all callbacks within same frame', () => {
        ...
        scheduleCallback(callbackC);
        scheduleCallback(callbackD);
        scheduleCallback(callbackE); // Initially doesn't call anything
        ...
        expect(callbackLog).toEqual(['A', 'B', 'C', 'D', 'E']); // errors should still get thrown
      ...
      it('and with some timed out callbacks, still calls all callbacks within same frame', () => {
        ...
        expect(callbackLog).toEqual(['C', 'D', 'E', 'A', 'B']); // errors should still get thrown
    ...
    describe('when all scheduled callbacks throw', () => {
      ...
      it('still calls all callbacks within same frame', () => {
        ...
        const postMessageErrorMessages = postMessageErrors.map(e => e.message);
        expect(postMessageErrorMessages).toEqual(['A error', 'B error', 'C error', 'D error', 'E error']);
        catchPostMessageErrors = false;
      });
      /**
       *                                  postMessage
    ...
    describe('when callbacks throw over multiple frames', () => {
      ...
      it('still calls all callbacks within same frame', () => {
        ...
        const callbackB = jest.fn(() => {
          callbackLog.push('B'); // time passes, causing us to run out of idle time
        ...
        const callbackD = jest.fn(() => {
          callbackLog.push('D'); // time passes, causing us to run out of idle time
        ...
        const callbackF = jest.fn(() => {
          callbackLog.push('F'); // time passes, causing us to run out of idle time
        ...
        scheduleCallback(callbackE);
        scheduleCallback(callbackF);
        scheduleCallback(callbackG); // does nothing initially

        expect(callbackLog).toEqual([]); // frame 1;
        ...
        expect(callbackLog).toEqual(['A', 'B']); // error was thrown from A

        let postMessageErrorMessages = postMessageErrors.map(e => e.message);
        expect(postMessageErrorMessages).toEqual(['A error']); // frame 2;
        ...
        expect(callbackLog).toEqual(['A', 'B', 'C', 'D']); // error was thrown from A

        postMessageErrorMessages = postMessageErrors.map(e => e.message);
        expect(postMessageErrorMessages).toEqual(['A error', 'C error']); // frame 3;
        ...
        expect(callbackLog).toEqual(['A', 'B', 'C', 'D', 'E', 'F']); // error was thrown from A

        postMessageErrorMessages = postMessageErrors.map(e => e.message);
        expect(postMessageErrorMessages).toEqual(['A error', 'C error', 'E error']); // frame 4;
        ...
        expect(callbackLog).toEqual(['A', 'B', 'C', 'D', 'E', 'F', 'G']); // error was thrown from A

        postMessageErrorMessages = postMessageErrors.map(e => e.message);
        expect(postMessageErrorMessages).toEqual(['A error', 'C error', 'E error']);
        catchPostMessageErrors = true;
      });
    });
  }); // TODO: test 'now'
